...
Java_com_localai_server_engine_LlamaEngine_nativeLoadModel(JNIEnv* env, jclass,
    ...
    fileLog("nativeLoadModel: cacheDir=%s", cacheDirStr.c_str());

    // Auto-detect optimal threads before load if nThreads <= 0
    auto bigCores = getBigCores();
    ...
    LOGI("nativeLoadModel: %s, nCtx=%d, nThreads=%d, cacheDir=%s", configPath.c_str(), nCtx, nThreads, cacheDirStr.c_str());
    ...
    // Set config: mmap for faster loading, threads, context and tmp_path for kvcache
    std::string configJson = "{\"num_ctx\":" + std::to_string(nCtx)
        + ",\"num_threads\":" + std::to_string(nThreads)
        + ",\"mmap\":true,\"use_mmap\":true"
        + ",\"attention_mode\":10,\"kvcache_mmap\":true}";
    fileLog("nativeLoadModel: set_config: %s", configJson.c_str());
    llm->set_config(configJson);

    // Set tmp_path for kvcache_mmap
